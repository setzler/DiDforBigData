...
#' @param varname_options A list of the form varname_options = list(id_var, time_var, outcome_var, cohort_var), where all four arguments of the list must be a character that corresponds to a variable name in inputdata.
...
#' @examples
#' varname_options = list(id_var = "id", time_var = "year", outcome_var = "price", cohort_var = "cohort")
#' DiD(inputdata=simdata, varname_options=varname_options, control_group = "future-treated", event_baseperiod=-2, min_event=-3, max_event=5)
#' @export
DiDge <- function(inputdata, varname_options, cohort_time, event_baseperiod, event_postperiod, control_group = "all"){
  
  # set up variable names
  time_var = varname_options$time_var
  outcome_var = varname_options$outcome_var
  cohort_var = varname_options$cohort_var
  id_var = varname_options$id_var
  
  # prepare time periods
  pre_time = cohort_time + event_baseperiod
  post_time = cohort_time + event_postperiod
  time_set = sort(inputdata[get(cohort_var) == cohort_time, unique(get(time_var))])
  ...
  inputdata = inputdata[get(time_var)==pre_time | get(time_var)==post_time]  
  inputdata[, nobs := .N, by=id_var]
  inputdata = inputdata[nobs==2 | (get(time_var)==pre_time & get(time_var)==post_time)]
  gc()
  
  # define treated data and get means
  treated_data_prepost = merge(
    inputdata[(get(cohort_var) == cohort_time) & (get(time_var) == pre_time), list(tmp_idvar=get(id_var),treated_pre_outcome=get(outcome_var))],
    inputdata[(get(cohort_var) == cohort_time) & (get(time_var) == post_time), list(tmp_idvar=get(id_var),treated_post_outcome=get(outcome_var))],
    by=c("tmp_idvar")
  )
  ...
  if(control_group == "all"){
    control_data_prepost = merge(
      inputdata[(get(cohort_var) > (cohort_time +  post_time)) & (get(time_var) == pre_time), list(tmp_idvar=get(id_var),control_pre_outcome=get(outcome_var))],
      inputdata[(get(cohort_var) > (cohort_time +  post_time)) & (get(time_var) == post_time), list(tmp_idvar=get(id_var),control_post_outcome=get(outcome_var))],
      by=c("tmp_idvar")
    )
  }
  if(control_group == "never-treated"){
    control_data_prepost = merge(
      inputdata[(get(cohort_var) > (cohort_time +  post_time)) & is.infinite(get(cohort_var)) & (get(time_var) == pre_time), list(tmp_idvar=get(id_var),control_pre_outcome=get(outcome_var))],
      inputdata[(get(cohort_var) > (cohort_time +  post_time)) & is.infinite(get(cohort_var)) & (get(time_var) == post_time), list(tmp_idvar=get(id_var),control_post_outcome=get(outcome_var))],
      by=c("tmp_idvar")
    )
  }
  if(control_group == "future-treated"){
    control_data_prepost = merge(
      inputdata[(get(cohort_var) > (cohort_time +  post_time)) & is.finite(get(cohort_var)) & (get(time_var) == pre_time), list(tmp_idvar=get(id_var),control_pre_outcome=get(outcome_var))],
      inputdata[(get(cohort_var) > (cohort_time +  post_time)) & is.finite(get(cohort_var)) & (get(time_var) == post_time), list(tmp_idvar=get(id_var),control_post_outcome=get(outcome_var))],
      by=c("tmp_idvar")
    )
  }
...
DiDe <- function(inputdata, varname_options, control_group = "all", event_baseperiod=-1, min_event=NULL, max_event=NULL){
  ...
  # set up and checks
  cohorts = sort(inputdata[, unique(get(cohort_var))])
  ...
  min_time_actual = inputdata[,min(get(time_var))]
  ...
  if(sum(invalid_cohorts) > 0){
    ...
    inputdata = inputdata[!(get(cohort_var) %in% cohorts_todrop)]
  }
  ...
  for(cc in cohorts){
    ...
    times_for_cohort = sort(inputdata[get(cohort_var) == cc, unique(get(time_var))])
    ...
    for(event_postperiod in event_periods){
      res = DiDge(inputdata, cohort_time = cc, event_baseperiod = event_baseperiod, event_postperiod = event_postperiod, 
