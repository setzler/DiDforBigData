...
PlotDiDge <- function(DiD_results, ci_factor = 1.96, max_jitter=0.05, lower_event=NULL, upper_event=NULL, colors=NULL){
  ...
  omitted_event_time = figdata_temp[,sort(unique(Preevent))]
  figdata_temp = figdata_temp[EventTime != Preevent]
...
PlotDiDe <- function(DiD_results, ci_factor = 1.96, max_jitter=0.05, lower_event=NULL, upper_event=NULL, colors=NULL){
  ...
  figdata_temp = figdata_temp[EventTime != Preevent]
  figdata_temp[, EventTime := as.numeric(EventTime)]
  figdata_temp = figdata_temp[ EventTime >= lower_event & EventTime <= upper_event]
...
#' @param upper_event Minimum event time (e) to show in the plot. Default option is NULL.
...
PlotEventsgt <- function(DiD_results, cohort=2007, ci_factor = 1.96, max_jitter=0.05, colors=NULL){
