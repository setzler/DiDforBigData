...
print(did$results_cohort[Cohort==2007][,1:6])
```
...
In difference-in-differences applications, it is often the case that treatment is assigned to groups of individuals (e.g., a change in  state-wide policy treats all individuals in a state simultaneously). If those groups are also subject to common shocks, this induces correlation in the estimation errors and the treatment assignment (see the discussion by Abadie, Athey, Imbens, and Wooldrige, 2022, QJE). ]

Fortunately, this is easy to fix: If the groups within which the estimation errors are correlated are known to the research, we just need to cluster standard errors by group. `DiDforBigData` requires only that the list of variable names, `varnames`, is modified to include a `cluster_names` entry. For example, `varnames$cluster_names = c("group1","group2")` tells it to cluster  in a way that accounts for common shocks to each of observable groups, "group1" and "group2".


### Example

There is an option in `SimDiD()` to group individuals into bins that are differentially selected for treatment and that also face common shocks within each bin:

```{r echo=T, eval=T, message=FALSE}

sim = SimDiD(sample_size=1000, bin_covars=TRUE)
simdata = sim$simdata
print(simdata)
...
We check the usual standard errors for cohort 2010:

```{r echo=T, eval=T, message=FALSE}
did = DiD(inputdata = simdata, varnames = varnames, min_event = -1, max_event=1)

print(did$results_average[,1:4])

```

Now, we cluster on the bin variable by adding it to the `varnames` and re-estimating:


```{r echo=T, eval=T, message=FALSE}

varnames$cluster_names = "bin" 
...
If you would like to use robust standard errors to match the ", robust" option from Stata, simply specify `robust=TRUE`.



### Example

We continue to use the simulated data from the previous example (with bins). We remove the clustering from the varnames. Instead, we set the `robust=TRUE` option:


```{r echo=T, eval=T, message=FALSE}
varnames$cluster_names = NULL

did = DiD(inputdata = simdata, varnames = varnames, min_event = -1, max_event=1, robust=TRUE)
