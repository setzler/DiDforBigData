...
### The `robust=TRUE` option and the `cluster_names` entry

In difference-in-differences applications, it is often the case that treatment is assigned to groups of individuals (e.g., a change in  state-wide policy treats all individuals in a state simultaneously). If those groups are also subject to common shocks, this induces correlation in the estimation errors, and standard errors will tend to be too small.
...
A related issue is allowing for the idiosyncratic errors to have difference variances for different individuals. We implement the equivalent of the ", robust" option from Stata using the option `robust=TRUE`.
...
sim = SimDiD(sample_size=1000, bin_covars_nobias = TRUE)
simdata = sim$simdata
print(simdata)
...
We check the usual standard errors:
...
Next, we try robust standard errors by adding the `robust=TRUE` option:


```{r echo=T, eval=T, message=FALSE}
did = DiD(inputdata = simdata, varnames = varnames, min_event = -1, max_event=3, robust=TRUE)
...
Finally, we cluster on the bin variable by adding it to the `varnames` and re-estimating:
